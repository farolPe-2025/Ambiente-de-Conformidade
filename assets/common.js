...
function fetchJSON(path){
  // "?v=" evita que o CDN do GitHub Pages sirva uma cópia em cache antiga
  // depois de uma publicação recente (cache:'no-store' só evita o cache local).
  var bustUrl = path + (path.indexOf('?')>=0?'&':'?') + 'v=' + Date.now();
...
/* Une uma lista publicada mais recente (de outro admin) com a lista local:
   mantém as edições/exclusões locais e só acrescenta itens novos publicados
   por outra pessoa que ainda não existem localmente. Evita que a edição de
   um admin "suma" quando outro publica por cima. */
function mergeById(remoteList, localList){
  remoteList = remoteList || []; localList = localList || [];
  var localIds = {};
  localList.forEach(function(l){ localIds[l.id] = true; });
  var merged = localList.slice();
  remoteList.forEach(function(r){ if(!localIds[r.id]) merged.push(r); });
  return merged;
}
/* opts.baseline: o que esta sessão sabe como "publicado" (capturado ao carregar a página).
   opts.merge(remoteData, localData): se informado, resolve conflitos automaticamente;
   se não, um conflito interrompe a publicação (nada é sobrescrito às cegas). */
function publishJSON(path, dataObj, message, opts){
  opts = opts || {};
  return promptForToken().then(function(token){
    ...
    return ghRequest(url, {}, token).then(function(r){ return r.ok ? r.json() : null; })
      .then(function(existing){
        var finalData = dataObj;
        if(existing && existing.content && opts.baseline !== undefined){
          var remoteJson = b64ToUtf8(existing.content);
          var remoteData = null;
          try{ remoteData = remoteJson===null ? null : JSON.parse(remoteJson); }catch(e){}
          var baselineStr = JSON.stringify(opts.baseline);
          if(remoteData !== null && JSON.stringify(remoteData) !== baselineStr){
            // Alguém publicou algo diferente do que esta sessão conhecia como "publicado".
            if(opts.merge){
              finalData = opts.merge(remoteData, dataObj);
              if(opts.onConflict) opts.onConflict(remoteData, finalData);
            } else {
              var ce = new Error('Alguém publicou uma alteração mais recente enquanto você editava. Recarregue a página para ver os dados atuais antes de publicar de novo (seu rascunho continua salvo neste navegador).');
              ce.conflict = true;
              throw ce;
            }
          }
        }
        var body = { message: message, content: utf8ToB64(JSON.stringify(finalData, null, 2)), branch: GITHUB_BRANCH };
...
function renderNav(activePage, opts){
  ...
  el.innerHTML = ''
    ...
    +   '<img src="assets/logo-icon-white.png" alt="Governo de Pernambuco" onerror="this.style.display=\'none\'">'
